...
class ARAR:
    ...
    def __init__(self, df, h, freq, max_lag=26):
        ...
        assert max_lag >= 26, "max_lag must be at least 26, i.e. max_lag >= 26"
    ...
    def forecast(self):
        ...
        for k in np.array(range(0, 3)):
            ...
            phi = [np.matmul(y[range(i+1, n)],
                             y[range(0, n-i-1)])/sum(
                                 np.power(y[range(
                                     0, n-i-1)], 2)) for i in range(0, 15)]

            err = [sum(np.power((y[range(i+1, n)]-phi[i]*y[range(
                0, (n - i-1))]), 2))/sum(np.power(y[range(
                    i, n)], 2)) for i in np.array(range(0, 15))]
            ...
            if(err[tau] <= 8/n or (phi[tau] >= 0.93 and tau > 3)):
                ...
                psi = np.append(np.append(psi, 0), 0) - np.array(
                    phi[1]) * np.append(np.append(0, psi), 0) - np.array(
                        phi[1]) * np.append(np.append(0, 0), psi)
            else:
                break
